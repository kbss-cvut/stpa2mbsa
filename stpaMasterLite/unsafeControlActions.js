...
function createUnsafeControlAction() {
  ...
  if (column < NOT_PROVIDING_UCA_COLUMN || column > DURATION_UCA_COLUMN || action.trim() === "") {
    ...
    SpreadsheetApp.getUi().alert(`Please select a cell within the D${UCA_SHEET_HEADER_ROW_COUNT + 1}:G${lastRow} range.`);
    return;
  }
  ...
  form.canAddNext = new String(ucaSheet.getRange(possibleNextCell.row, ACTION_COLUMN).getValue()).trim().length > 0;
  SpreadsheetApp.getUi().showModalDialog(form.evaluate().setWidth(550).setHeight(300 + form.referencesSelectHeight + (form.existing.length > 0 ? 12 : 0)), 'New Unsafe Control Action');
}
...
function onCreateUnsafeControlActionSubmit(data) {
  ...
  if (isExistingUCAValueEmpty(existingValue)) {
    ...
    if (data.overwriteType === "one") {
      ...
      const indexToOverwrite = ucas.findIndex(u => u.startsWith(data.overwrite));
      ucas.splice(indexToOverwrite, 1, uca);
      cell.setValue(ucas.join("\n\n"));
    } else {
      cell.setValue(`${existingValue}${existingValue.length > 0 ? "\n\n" : ""}${uca}`);
    }
  }
  cell.setHorizontalAlignment("left");
  cell.setWrap(true);
  generateControllerConstraint(data.ucaDefinition, ucaId, cell, data.overwriteType);
...
function resolveUnsafeControlActionIndex(currentCellValue, toOverwrite, overwriteType) {
  if (currentCellValue.length > 0 && currentCellValue !== "N/A" && overwriteType === "one") {
   ...
   const range = ucaSheet.getRange(`D${UCA_SHEET_HEADER_ROW_COUNT + 1}:G${lastRow}`);
    ...
    for (let y = 1; y <= rowCount; y++) {
      ...
      if (value !== "N/A" && value !== "") {
        ...
        for (let i = 0; i < matches.length; i++) {
          const m = Number(matches[i].substring(matches[i].indexOf('-') + 1, matches[i].length - 1));
          if (m > maxId) {
            maxId = m;
          }
        }
...
function generateControllerConstraint(uca, ucaId, ucaCell, overwriteType) {
  ...
  switch (ucaColumn) {
    ...
    case TEMPORAL_UCA_COLUMN:
      ...
      break;
    case DURATION_UCA_COLUMN:
      constraint = uca.replace("stops providing", "must not stop providing").replace("applies", "must not apply");
      break;
    default:
      constraint = "";
      break;
  ...
  if (overwriteType === "none") {
    ...
    targetCell.setValue(`${existingValue}${existingValue.length > 0 ? "\n\n" : ""}(${constraintId})\n${constraint}\n[${ucaId}]`);
  } else {
    targetCell.setValue(replaceControllerConstraint(constraintId, `(${constraintId})\n${constraint}\n[${ucaId}]`, targetCell));
  }
...
function replaceControllerConstraint(constraintId, constraint, cell) {
  ...
  const indexToOverwrite = constraints.findIndex(c => c.startsWith("(" + constraintId + ")"));
...
function getControlStructureInfo(ucaCell) {
  const row = ucaCell.getRow();
  const controlAction = ucaCell.getSheet().getRange(row, ACTION_COLUMN).getValue();
  ...
  const controlledProcess = SpreadsheetApp.getActive().getSheetByName(CS_SHEET_NAME).getRange(row, 3).getValue();
  return {
    controller,
    controlAction,
    controlledProcess
    ...
    for (let x = 1; x <= columnCount; x++) {
     ...
     if (ucaStr !== "N/A") {
       ...
       ucas.forEach(uca => {
        ...
        ucaIds.add(ucaId);
        validators.forEach(v => v(uca, ucaId, violations, {row: y + UCA_SHEET_HEADER_ROW_COUNT, column: x + UCA_SHEET_SCS_COLUMN_COUNT}));
        validateConstraintExistsForUnsafeControlAction(ucaId, y + UCA_SHEET_HEADER_ROW_COUNT, violations, {column: x + UCA_SHEET_SCS_COLUMN_COUNT, row: y + UCA_SHEET_HEADER_ROW_COUNT});
       })
     }
   }
  }
...
function restoreUnsafeControlActionsCellsBackground() {
  ...
  sheet.getRange(`D${UCA_SHEET_HEADER_ROW_COUNT}:K${STEP_THREE_MAX_EXPECTED_ROWS}`).setBackground(WHITE_BACKGROUND);
}
...
function validateConstraintExistsForUnsafeControlAction(ucaId, rowNumber, violations, cell) {
  const constraints = SpreadsheetApp.getActive().getSheetByName(UCA_SHEET_NAME).getRange(`H${rowNumber}:Z${rowNumber}`).getValues()[0];
  let found = false;
...
function validateUcaExistsForEachConstraint(ucaIds, lastRow, violations) {
  const allConstraints = SpreadsheetApp.getActive().getSheetByName(UCA_SHEET_NAME).getRange(`H${UCA_SHEET_HEADER_ROW_COUNT + 1}:K${lastRow}`).getValues();
