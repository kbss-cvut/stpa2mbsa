  ...
  Object.keys(controlStructure).sort().forEach(controller => {
      ...
      items.forEach(item => {
        ...
        actions.forEach(a => {
          ...
          const controlledProcessRange = ucaSheet.getRange(lastRow, CONTROLLED_PROCESS_COLUMN);
      ...
      const controllerRange = ucaSheet.getRange(`A${controllerRow}:A${lastRow - 1}`);
      ...
      const csControllerRange = csSheet.getRange(`A${controllerRow}:A${lastRow - 1}`);
