...
function addLoss() {
  ...
  SpreadsheetApp.getUi().showModalDialog(
    form.evaluate().setWidth(520).setHeight(150),
    "New Loss",
  );
}
...
function onLossSubmit(data) {
  ...
  const lastRow =
    sheet
      .getRange(`A${PURPOSE_HEADER_ROW_COUNT + 1}:A`)
      .getValues()
      .filter(String).length + PURPOSE_HEADER_ROW_COUNT;
  const nextRow = lastRow + 1;
  sheet
    .getRange(nextRow, 1)
    .setValue("L-" + (nextRow - PURPOSE_HEADER_ROW_COUNT));
...
function addSystemLevelHazard() {
  ...
  SpreadsheetApp.getUi().showModalDialog(
    form.evaluate().setWidth(520).setHeight(300),
    "New System-level Hazard",
  );
}
...
function onSystemLevelHazardSubmit(data) {
  ...
  if (data.parent === "none" || data.parent === undefined) {
    ...
    if (hazardIds.length === 0) {
      ...
      const highestHazardNumber = Number(
        lastId.substring(
          2,
          lastId.indexOf(".") !== -1 ? lastId.indexOf(".") : lastId.length,
        ),
      );
    ...
    shiftHazardRowsIfNecessary(
      sheet,
      hazardIds.length + PURPOSE_HEADER_ROW_COUNT + 1,
      targetRow,
    );
    if (parentId === closestSibling) {
      hazardId = parentId + ".1";
    } else {
      const closestSiblingNumber = Number(
        closestSibling.substring(
          closestSibling.lastIndexOf(".") + 1,
          closestSibling.length,
        ),
      );
  ...
  sheet
    .getRange(targetRow, HAZARD_LOSS_REFS_COLUMN)
    .setValue(`[${data.losses.join(", ")}]`);
  if (data.addAnother) {
...
function shiftHazardRowsIfNecessary(sheet, lastRow, targetRow) {
  ...
  while (row >= targetRow) {
    sheet
      .getRange(row + 1, HAZARD_ID_COLUMN)
      .setValue(sheet.getRange(row, HAZARD_ID_COLUMN).getValue());
    sheet
      .getRange(row + 1, HAZARD_COLUMN)
      .setValue(sheet.getRange(row, HAZARD_COLUMN).getValue());
    sheet
      .getRange(row + 1, HAZARD_LOSS_REFS_COLUMN)
      .setValue(sheet.getRange(row, HAZARD_LOSS_REFS_COLUMN).getValue());
    row--;
  }
}
...
function addSystemLevelConstraint() {
  ...
  SpreadsheetApp.getUi().showModalDialog(
    form.evaluate().setWidth(520).setHeight(250),
...
function extractSystemLevelHazards() {
  const sheet = SpreadsheetApp.getActive().getSheetByName(STEP_ONE_SHEET_NAME);
  const hazardIds = sheet
    .getRange(`D${PURPOSE_HEADER_ROW_COUNT + 1}:D`)
    .getValues()
    .filter(String);
  if (hazardIds.length === 0) {
    return [];
...
function onSystemLevelConstraintSubmit(data) {
  const sheet = SpreadsheetApp.getActive().getSheetByName(STEP_ONE_SHEET_NAME);
  const constraintIds = sheet
    .getRange(`H${PURPOSE_HEADER_ROW_COUNT + 1}:H`)
    .getValues()
    .filter(String);
  const targetRow = constraintIds.length + PURPOSE_HEADER_ROW_COUNT + 1;
  sheet
    .getRange(targetRow, CONSTRAINT_ID_COLUMN)
    .setValue("SC-" + (constraintIds.length + 1));
  sheet.getRange(targetRow, CONSTRAINT_COLUMN).setValue(data.description);
  sheet
    .getRange(targetRow, CONSTRAINT_HAZARD_REFS_COLUMN)
    .setValue(`[${data.hazards.join(", ")}]`);
  if (data.addAnother) {
...
function validateStepOne() {
  ...
  const losses = sheet
    ...
    .map((row, i) => ({
      value: row[0],
      column: LOSS_ID_COLUMN,
      row: i + 1 + PURPOSE_HEADER_ROW_COUNT,
    }));
  const hazards = sheet
    .getRange(`D${PURPOSE_HEADER_ROW_COUNT + 1}:D`)
    .getValues()
  ...
  const hazardLossRefs = sheet
    .getRange(`F${PURPOSE_HEADER_ROW_COUNT + 1}:F`)
    .getValues()
    .filter(String)
    .map((row, i) => ({
      value: row[0],
      column: HAZARD_LOSS_REFS_COLUMN,
      row: i + 1 + PURPOSE_HEADER_ROW_COUNT,
    }));
  const constraints = sheet
    .getRange(`H${PURPOSE_HEADER_ROW_COUNT + 1}:H`)
    .getValues()
    .filter(String)
    .map((row, i) => ({
      value: row[0],
      column: CONSTRAINT_ID_COLUMN,
      row: i + 1 + PURPOSE_HEADER_ROW_COUNT,
    }));
  const constraintHazardRefs = sheet
    .getRange(`J${PURPOSE_HEADER_ROW_COUNT + 1}:J`)
    .getValues()
    .filter(String)
    .map((row, i) => ({
      value: row[0],
      column: CONSTRAINT_HAZARD_REFS_COLUMN,
      row: i + 1 + PURPOSE_HEADER_ROW_COUNT,
    }));
  ...
  violations = violations.concat(
    validateIdUniqueness(hazards, "System-level hazard"),
  );
  violations = violations.concat(
    validateIdUniqueness(constraints, "System-level constraint"),
  );
...
function restoreStepOneCellsBackground() {
  const sheet = SpreadsheetApp.getActive().getSheetByName(STEP_ONE_SHEET_NAME);
  sheet
    .getRange(`A${PURPOSE_HEADER_ROW_COUNT + 1}:A${STEP_ONE_MAX_EXPECTED_ROWS}`)
    .setBackground(NON_EDITABLE_BACKGROUND);
  sheet
    .getRange(`D${PURPOSE_HEADER_ROW_COUNT + 1}:D${STEP_ONE_MAX_EXPECTED_ROWS}`)
    .setBackground(NON_EDITABLE_BACKGROUND);
  sheet
    .getRange(`F${PURPOSE_HEADER_ROW_COUNT + 1}:F${STEP_ONE_MAX_EXPECTED_ROWS}`)
    .setBackground(NON_EDITABLE_BACKGROUND);
  sheet
    .getRange(`H${PURPOSE_HEADER_ROW_COUNT + 1}:H${STEP_ONE_MAX_EXPECTED_ROWS}`)
    .setBackground(NON_EDITABLE_BACKGROUND);
  sheet
    .getRange(`J${PURPOSE_HEADER_ROW_COUNT + 1}:J${STEP_ONE_MAX_EXPECTED_ROWS}`)
    .setBackground(NON_EDITABLE_BACKGROUND);
}
    ...
    if (v.row) {
      if (v.sheet) {
        SpreadsheetApp.getActive()
          .getSheetByName(v.sheet)
          .getRange(v.row, v.column)
