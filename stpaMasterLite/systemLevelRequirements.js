...
function addSystemLevelRequirement() {
  ...
  const form = HtmlService.createTemplateFromFile('createSystemLevelRequirement');
  const lossScenarios = getLossScenarios();
  form.lossScenarios = lossScenarios;
  form.lossScenariosHeight = Object.keys(lossScenarios).length * SELECT_OPTION_HEIGHT + SELECT_PADDING;
  form.existing = {
    references: []
  };
  SpreadsheetApp.getUi().showModalDialog(form.evaluate().setWidth(720).setHeight(280 + form.lossScenariosHeight), 'New System-level Requirement');
}

function updateSystemLevelRequirement() {
  const range = SpreadsheetApp.getActiveRange();
  if (range == null || range.getSheet().getName() !== SLR_SHEET_NAME) {
    SpreadsheetApp.getUi().alert("Select a requirement in the " + SLR_SHEET_NAME + " spreadsheet.");
    return;
  }
  const row = range.getRow();
  const sheet = SpreadsheetApp.getActive().getSheetByName(SLR_SHEET_NAME);
  const itemCount = sheet.getRange(`A${SLR_SHEET_HEADER_ROWS + 1}:A`).getValues().filter(String).length;
  ...
  form.existing = {
    ...
    references: resolveExistingSystemLevelRequirementReferences(sheet.getRange(row, 3).getValue()),
    row
  };
  SpreadsheetApp.getUi().showModalDialog(form.evaluate().setWidth(720).setHeight(280 + form.lossScenariosHeight), 'Update System-level Requirement');
}
...
function validateSystemLevelRequirements() {
  ...
  const sheet = SpreadsheetApp.getActive().getSheetByName(SLR_SHEET_NAME);
  const lossScenarios = getLossScenarios();
  const scenarioReferences = sheet.getRange(`C${SLR_SHEET_HEADER_ROWS + 1}:C`).getValues().filter(String).map(row => row[0]);
  let violations = validateLossScenariosReferencedBySystemLevelRequirements(lossScenarios, scenarioReferences);
  violations = violations.concat(validateSystemLevelRequirementsReferenceExistingScenarios(Object.keys(lossScenarios), scenarioReferences, sheet));
...
function restoreSystemLevelRequirementsCellsBackground() {
  ...
  slrSheet.getRange(`A${SLR_SHEET_HEADER_ROWS}:A${SLR_MAX_EXPECTED_ROWS}`).setBackground(NON_EDITABLE_BACKGROUND);
  slrSheet.getRange(`C${SLR_SHEET_HEADER_ROWS}:C${SLR_MAX_EXPECTED_ROWS}`).setBackground(NON_EDITABLE_BACKGROUND);
}

function validateLossScenariosReferencedBySystemLevelRequirements(lossScenarios, scenarioReferences) {
  const scenariosSheet = SpreadsheetApp.getActive().getSheetByName(LOSS_SCENARIOS_SHEET_NAME);
  const ucas = scenariosSheet.getRange(`A${LOSS_SCENARIOS_HEADER_ROWS + 1}:A`).getValues();
  const lastRow = ucas.filter(String).length + UCA_SHEET_HEADER_ROW_COUNT;
  const range = scenariosSheet.getRange(LOSS_SCENARIOS_HEADER_ROWS + 1, LOSS_SCENARIO_TYPE_ONE_COLUMN, lastRow, 4);
  return Object.keys(lossScenarios).filter(id => {
...
function validateSystemLevelRequirementsReferenceExistingScenarios(scenarioIds, scenarioReferences, sheet) {
  const violations = [];
  scenarioReferences.forEach((cell, index) => {
    ...
    references.filter(r => scenarioIds.indexOf(r.trim()) === -1).forEach(r => violations.push({
